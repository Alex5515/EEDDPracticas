...
int main(int argc, const char * argv[]) {
    ...
    VDinamico<PaMedicamento*> encontrados= vMedicamentos.buscarCompuesto("ACEITE",vMedicamentos);

    for (unsigned int i = 0; i < encontrados.tamlog(); ++i) {
        PaMedicamento* ptrMed = encontrados[i];
        std::cout << "ID: " << ptrMed->getIdNum() << " - " << ptrMed->getNombre() << std::endl;
    }
        /**
            std::ifstream is;
