//
// Created by Fernando SCarrion on 23/09/2025.
//
...
public:
    ...
    PaMedicamento(int id, const string& alpha, const string& n)
        : IdNum(id), IdAlpha(alpha), nombre(n) {}

    int getIdNumber() const { return IdNum; }
    string getIdAlpha() const { return IdAlpha; }
    string getNombre() const { return nombre; }

    void setIdNumber(int id) { IdNum = id; }
    void setIdAlpha(const std::string& alpha) { IdAlpha = alpha; }
    void setNombre(const std::string& n) { nombre = n; }

    bool operator<(const PaMedicamento& otro) const {
        return IdNum < otro.IdNum;
    }
};
...
template <typename T>
    ...
    * */
    VDinamico(const VDinamico<T> &orig, unsigned int posInicial, unsigned int numElem) : _tamfis(orig._tamfis), _tamlog(orig._tamlog) {
        ...
        if (_tamfis != n) {
            throw std::invalid_argument("[VDinamico<T>::VDinamico<T>]: El tam fisico de no es potencia de 2");
        }
        //Compruebo si se ha pasado el tamaño lógico
        if (numElem + posInicial > orig._tamlog) {
            throw std::out_of_range("[VDinamico<T>::VDinamico<T>]:Se ha superado el tam logico del vector");
        }
