...
* */
    ...
    /**
     * @brief BuscarCompuesto
     * @param buscamos la cadena indicada en cada medicamento
    * @return devuelve un vector con los punteros a los objetos que contienen ACEITE
    * */

    VDinamico<T*> buscarCompuesto(const string& cadena, VDinamico<T>& vMedicamentos) { VDinamico<T*> resultado;
        for (unsigned int i = 0; i < vMedicamentos.tamlog(); ++i) {
            string nombre = vMedicamentos[i].getNombre();
            if (nombre.find(cadena) != string::npos) { //npos lo usamos para comprobar que la cadena no esta vacia, funciona solo con el find
