...
MediExpress::MediExpress(const std::string& file_meds, const std::string& file_labs) {
    ...
    std::cout << "Inicializacion de datos completada y medicamentos enlazados con los laboratorios" << std::endl;
}
...
void MediExpress::cargarLaboratorios(const std::string& filename) {
    ...
    while (getline(file, line)) {
        std::stringstream ss(line);
        std::string id_str, nombre, dir, cp, prov, pais, tel;
        getline(ss, id_str, ';');
        getline(ss, nombre, ';');
        getline(ss, dir, ';');
        getline(ss, cp, ';');
        ...
        getline(ss, pais, ';');
        getline(ss, tel, ';');
        laboratorios.insertarFinal(Laboratorio(std::stoi(id_str), nombre, dir, cp, prov));
    }
...
void MediExpress::MostrarLabsGranada() {
...
void MediExpress::LabsJaen() {
    ...
    std::cout << "Hay " << contador2 << " laboratorios en Jaen" << std::endl;
}

void MediExpress::MostrarLaboratoriosMadrid() {
...
void MediExpress::LabsAMedicamentosSinSuministrador() {
...
void MediExpress::eliminarLabsBruselas() {
    ...
    while (it_busqueda != laboratorios.end()) {
        std::string CP = (*it_busqueda).getCp();
        ...
        if (CP.find("bruselas") != std::string::npos || provincia.find("bruselas") != std::string::npos) {
